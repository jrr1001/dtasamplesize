...
#'     \item{N_total_median}{Median total N accounting for prevalence
#'       uncertainty and for both the sensitivity (diseased) and
#'       specificity (non-diseased) sample-size requirements.}
...
bam_sample_size <- function(prior_se = c(17, 3),
  ...
  # The total sample must supply BOTH n_se diseased and n_sp non-diseased
  # subjects; for a given prevalence draw the binding requirement is the
  # larger of n_se / prev and n_sp / (1 - prev).
  set.seed(seed)
